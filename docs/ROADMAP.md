...
- 📋 **VDS77** (deps: —) **react-hook-form is a dependency, so a consumer can get a second copy of the one library here carrying a context** — The ranges overlap today, so the single copy is a coincidence rather than a guarantee. → §VDS77
