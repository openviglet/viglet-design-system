...
### §VDS45 One way to ship a stylesheet

A stylesheet leaves this package four different ways: `viglet-design-system.css` is
built with every entry's CSS merged into it, `preset.css` is `cpSync`ed in the build
script, `bento.css` and `floating-formulas-bg.css` are copied by a Vite plugin, and
`fonts.css` is generated by `scripts/build-fonts.mjs`.

The fourth exists only because of the first. `cssCodeSplit: false` merges every entry's
CSS, so adding a fonts entry put the faces back into `./styles` - which is what VDS41
had just taken out - and the faces had to be assembled by hand instead. Setting
`cssCodeSplit: true` and building emits `index.css`, `bento.css` and
`floating-formulas-bg.css` per entry, which is what two of the copies produce by hand.

`build-fonts.mjs` also restates which families to ship, duplicating `fonts.css` - the
same defect VDS43 removed from the externals, reintroduced in the same session.

Check what the split changes about what a consumer downloads before adopting it: per
entry is not automatically less than merged, and `./styles` would be renamed from
`viglet-design-system.css` to `index.css` behind the same subpath.
